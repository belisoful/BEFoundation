...
/// Returns YES if a * b would overflow size_t.
static inline BOOL BEMulOverflowsSize(size_t a, size_t b) {
							  ...
							  rgbaRowBytes:(size_t)rgbaRowBytes
{
	...
	if (BEMulOverflowsSize(height, float32RowBytes)) {
		return NO;
	}
...
+ (nullable BEImage *)imageFromTexture:(id<MTLTexture>)texture
{
	...
	switch (texture.pixelFormat) {
		...
		case MTLPixelFormatR8Unorm:
			bitsPerComponent = 8;
			bytesPerPixel = 1;
			bitmapInfo = (CGBitmapInfo)kCGImageAlphaNone | kCGBitmapByteOrder32Big;
			needsConversion = YES;
			break;
		case MTLPixelFormatR16Float:
			bitsPerComponent = 16;
			bytesPerPixel = 2;
			bitmapInfo = (CGBitmapInfo)kCGImageAlphaNone | kCGBitmapFloatComponents;
			needsConversion = YES;
			break;
	...
	if (width == 0 || height == 0 ||
		BEMulOverflowsSize(bytesPerPixel, width) ||
		BEMulOverflowsSize(height, bytesPerPixel * width)) {
		return nil;
	}
	...
	// Expand single-channel formats to RGBX via vImage.
	if (needsConversion) {
		NSUInteger argbBytesPerRow;
		NSUInteger newBitsPerComponent;
		NSUInteger newBytesPerPixel;
		...
		if (BEMulOverflowsSize(newBytesPerPixel, width) ||
			BEMulOverflowsSize(height, newBytesPerPixel * width)) {
			free(imageBytes);
			CGColorSpaceRelease(colorSpace);
			return nil;
		}
		argbBytesPerRow = newBytesPerPixel * width;

		void *argbBytes = malloc(height * argbBytesPerRow);
		if (!argbBytes) {
			free(imageBytes);
			CGColorSpaceRelease(colorSpace);
			return nil;
		...
		switch (texture.pixelFormat) {
			case MTLPixelFormatR8Unorm:
				ok = [BEMetalHelper convertGray8toXRGB8888WithVImage:imageBytes
															   width:width
															  height:height
				...
				break;
			case MTLPixelFormatR16Float:
				ok = [BEMetalHelper convertGray16FtoRGBXFFFFWithVImage:imageBytes
																 width:width
																height:height
															  rowBytes:imageBytesPerRow
																 alpha:1.0
				...
				break;
			case MTLPixelFormatR32Float:
				ok = [BEMetalHelper convertGray32FtoRGBXFFFFWithVImage:imageBytes
																 width:width
																height:height
															  rowBytes:imageBytesPerRow
																 alpha:1.0
...
#if TARGET_OS_OSX
	BEImage *image = [[NSImage alloc] initWithCGImage:cgImage size:NSMakeSize(width, height)];
#else
	BEImage *image = [UIImage imageWithCGImage:cgImage];
#endif

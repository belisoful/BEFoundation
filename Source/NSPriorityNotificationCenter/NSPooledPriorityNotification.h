...
/*!
 ...
 @discussion	This pools notifications for observers that use queue. This is to ensure that a notification
				...
				@code
				// Borrow a pooled notification, deliver it, then return it to the pool.
				NSPooledPriorityNotification *notif =
					[NSPooledPriorityNotification newTempNotificationWithName:notif.name
																	  object:notif.object
																	userInfo:notif.userInfo
																	 reverse:notif.reverse];
				[observer performSelector:selector withObject:notif];
				[notif recycle];
				@endcode

...
{
	self = [self initWithName:name object:object userInfo:userInfo reverse:reverse postBlock:nil];
	return self;
}
...
{
	self = [self initWithName:name object:object userInfo:userInfo reverse:NO postBlock:postBlock];
	return self;
}
...
// which is why the missing-super-call warning is suppressed.
...
+ (NSSet<Class> *)userInfoCodingClasses
{
	...
	dispatch_once(&once, ^{
		classes = [NSSet setWithObjects:NSDictionary.class, NSArray.class, NSSet.class,
				   NSString.class, NSNumber.class, NSDate.class, NSData.class,
				   NSNull.class, NSURL.class, nil];
	});
	return classes;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
	if (self) {
		if ([aDecoder allowsKeyedCoding]) {
			...
			_userInfo = [aDecoder decodeObjectOfClasses:[NSPriorityNotification userInfoCodingClasses] forKey:@"userInfo"];
			...
			NSNumber *extraProperties = [aDecoder decodeObject];
			if (extraProperties.integerValue & (1 << 0)) {
				self.tag = [[aDecoder decodeObject] integerValue];
			}
			if (extraProperties.integerValue & (1 << 1)) {
				self.identifier = [aDecoder decodeObject];
			}
		}

 ...
 *
 *             @code
 *             // Swap an instance method with its replacement on a target class.
 *             BEDynamicMethodSwizzleSelectors *swizzle =
 *                 [BEDynamicMethodSwizzleSelectors swizzleOriginal:@selector(description)
 *                                                     withSelector:@selector(be_description)];
 *             int result = [swizzle swizzleMethodsOnClass:[MyClass class]];   // 1 exchanged, -1 added, 0 failed
 *             BOOL active = [BEDynamicMethodSwizzleSelectors statusClassIsSwizzled:[MyClass class]];
 *             @endcode
 */

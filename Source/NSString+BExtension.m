...
/*!
 ...
 @discussion	Parses the string using the current system's default short date and time format
				(as determined by locale settings).
...
/*!
 ...
 @discussion	Parses the string using the current system's default short date format
				(as determined by locale settings).
...
/*!
 ...
 @discussion	Parses the string using the current system's default short time format
				(as determined by locale settings).
...
- (nonnull NSString *)stringByPrependingString:(NSString * _Nonnull)aString
{
	if(!aString) {
		[NSException raise:NSInvalidArgumentException
								format:@"*** -[%@ %@]: nil argument",
									   NSStringFromClass(NSString.class), NSStringFromSelector(_cmd)];
...
*/
- (NSUInteger)countCharactersInSet:(NSCharacterSet *)set
{
	return [self countCharactersInSet:set range:NSMakeRange(0, self.length)];
}

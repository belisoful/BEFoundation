...
/*!
 ...
 @discussion Decodes byte-wise so the result carries the payload bytes in the URL's declared
			 ...
			 NSURL repairs a malformed escape while it builds the URL, rewriting "%ZZ" as
			 "%25ZZ", and the repair is lossy: the original text is gone by the time any
			 accessor runs. A payload read from an NSURL therefore never reaches the nil
			 returns below. They hold for input that arrives by another route, and for
			 platforms whose NSURL leaves the escape intact.
...
- (BOOL)parseDataUrl:(NSString *)resourceSpecifier mimeType:(NSString**)mimeType charset:(NSString**)charset encoding:(NSStringEncoding*)encoding isBase64:(BOOL*)base64
{
	...
	if (commaRange.location == NSNotFound) {
		...
		// leaves its memo alone: overwriting it would make the property
		// disagree with +isDataURL: and flip with call order.
		if (mimeType)	*mimeType = nil;
		if (charset)	*charset = nil;
		if (encoding)	*encoding = 0;
		if (base64)		*base64 = NO;
...
/*!
 ...
 @discussion Decodes the data URL content based on its encoding:
			 ...
			 A malformed escape decodes to its literal characters: NSURL rewrites "%ZZ" as
			 "%25ZZ" while building the URL, and that escape decodes back to "%ZZ". To
			 reject such a URL instead, build it with
			 +URLWithString:encodingInvalidCharacters: passing NO (macOS 14, iOS 17), which
			 returns nil for the malformed string.

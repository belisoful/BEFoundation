...
- (nullable NSURL *)startAccessingURL:(NSURL *)url {
	...
	if (!resolvedURL && [self.delegate respondsToSelector:@selector(securityScopedURLManager:accessFailedForURL:entry:completionHandler:)]) {
		if ([NSThread isMainThread]) {
			// The handler applies the relocation itself, so a URL delivered after this method
			// returns (e.g. once an NSOpenPanel sheet is dismissed) still grants access,
			// re-keys the catalog and fires didRelocate. Only this call's return value is
			// unavailable in that case, since it has already returned.
			__block BOOL answeredInline = NO;
			[self.delegate securityScopedURLManager:self
				...
				answeredInline = YES;
				if (relocatedURL) {
					NSURL *granted = [self finishRelocationForURL:url entry:entry delegateURL:relocatedURL];
					if (granted) {
						resolvedURL = granted;    // read below only when answeredInline is YES
					}
				}
			}];
			if (!answeredInline) {
				BELogError(@"[BESecurityScopedURLManager] delegate has not answered the "
						   @"completion handler for %@ synchronously on the main thread; this "
						   @"call returns no access. The relocation still applies when the "
			...
			if (delegateURL) {
				resolvedURL = [self finishRelocationForURL:url entry:entry delegateURL:delegateURL];
			}
		}
	}
...
/*!
 @method        finishRelocationForURL:entry:delegateURL:
 @abstract      Applies a delegate-supplied relocation URL: starts access, refreshes the
				bookmark, re-keys the catalog entry, and fires didRelocateURL:toURL:.
 @discussion    Split out of -startAccessingURL: so the relocation can be applied from the
 ...
 @param         delegateURL The replacement URL the delegate provided.
 @return        The granted (relocated) URL, or nil if access could not be started.
 */
- (nullable NSURL *)finishRelocationForURL:(NSURL *)url
									 entry:(BESecurityScopedURLBookmarkEntry *)entry
							   delegateURL:(NSURL *)delegateURL {
	__block NSURL *granted = nil;
	__block BOOL success = NO;

	// This dispatch_sync is safe: -startAccessingURL:'s own dispatch_sync completed before the
	...
	if (success && ![delegateURL.absoluteString isEqualToString:url.absoluteString]) {
		if ([self.delegate respondsToSelector:@selector(securityScopedURLManager:didRelocateURL:toURL:)]) {
			// The header promises main-thread delegate delivery; this path can run on a
			// background thread (the semaphore branch of startAccessingURL:).
			dispatch_async(dispatch_get_main_queue(), ^{
				[self.delegate securityScopedURLManager:self
										 didRelocateURL:url
												  toURL:delegateURL];
...
- (NSArray<NSURL *> *)startAccessingAllURLs {
	...
	dispatch_sync(self.accessQueue, ^{
		for (BESecurityScopedURLBookmarkEntry *entry in self.mutableCatalog.allValues) {
			// Feed the catalog key in, not entry.url: the resolver keys resolvedAccessURLByKey
			// by what it is given, so passing an already-resolved URL misses the Tier 1 lookup
			// and records the wrong key. Using the key also avoids eagerly resolving every
			// bookmark here.
			NSURL *keyURL = [NSURL URLWithString:entry.urlString];
			if (keyURL) {
				NSURL *accessedURL = [self startAccessingURLInternal:keyURL];
				if (accessedURL) {

...
{
	// Stored per class object, so BECharacterSet and BEMutableCharacterSet each carry their
	// own setting: setting one does not change the other.
...
- (id)copyWithZone:(nullable NSZone *)zone
{
	...
	// Carry the per-instance equality override across: a copy that silently reverts to the
	// class default compares differently from the object it was copied from.
	BECharacterSet *copy = [[BECharacterSet allocWithZone:zone] initWithSet:_characterSet];
	copy.isEqualToNSCharacterSet = _isEqualToNSCharacterSet;
	return copy;
}
...
{
	BEMutableCharacterSet *copy = [[BEMutableCharacterSet allocWithZone:zone] initWithSet:_characterSet];
	copy.isEqualToNSCharacterSet = _isEqualToNSCharacterSet;
	return copy;
}
...
{
	NSCharacterSet *fileSet = [NSCharacterSet characterSetWithContentsOfFile:fName];
	if (!fileSet) {
		return nil;	// the declared contract: nil when the file cannot be read
	}
	return [[BECharacterSet alloc] initWithSet:fileSet];
}
...
{
	NSMutableCharacterSet *fileSet = [NSMutableCharacterSet characterSetWithContentsOfFile:fName];
	if (!fileSet) {
		return nil;	// the declared contract: nil when the file cannot be read
	}
	return [[BEMutableCharacterSet alloc] initWithSet:fileSet];
}

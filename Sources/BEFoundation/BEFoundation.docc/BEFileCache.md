...
| `1` | Pure value density; score ties evict least-recently-used first. |
...
cache.fileNameBlock = ^NSString *(id<NSCopying, NSSecureCoding> key, NSString *hashName) {
    // Readable prefix + digest prefix for guaranteed uniqueness.
    return [NSString stringWithFormat:@"avatar-%@", [hashName substringToIndex:16]];
};
```

...
The round-trip and data members use representation-style names (`imageFromCGImage:`, `pngRepresentation`, following the `TIFFRepresentation` idiom). Apple frameworks attach private same-named category methods to these classes at runtime (PencilKit, when loaded, adds `+[NSImage imageWithCGImage:]` and `-CGImage`), and which duplicate method wins is undefined, so a category on a framework class must not reuse Apple's method names. The factories return `nil` for a `NULL`/`nil` input on both platforms.

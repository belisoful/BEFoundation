...
/*!
 @method        showWindow:
 @abstract      Re-announces a window that has already loaded so it re-registers with the manager.
 @discussion    -windowDidLoad fires once, when the nib loads. Closing a window untracks the
				controller in BEWindowControllerManager (via NSWindowWillCloseNotification), but
				re-showing it reuses the loaded window without a second -windowDidLoad, so the
...
- (void)showWindow:(nullable id)sender {
	...
	if (wasLoaded && self.window) {
		[[NSNotificationCenter defaultCenter] postNotification:
			[NSNotification notificationWithName:BEWindowDidLoadNotification object:self.window]];
	}
}

...
- (void)removeTabViewItem:(NSTabViewItem *)tabViewItem
{
	@synchronized (self) {
		...
		BOOL wasHidden = tabViewItem.hidden;

		// Update the backing store before the delegate is notified, so a delegate that reads
		// allTabViewItems / numberOfAllTabViewItems from its change callback sees the removal.
		...
		if (!wasHidden) {
			...
			// A hidden tab is not in the visible set, so super would not fire the delegate.
			id<BETabViewDelegate> delegate = (id<BETabViewDelegate>)self.delegate;

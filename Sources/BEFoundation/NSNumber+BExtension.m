/*!
 ...
 @abstract		Implements the NSNumber (Extension) type-aware arithmetic and the pow_int64/pow_uint64 helpers.
 @discussion	numberOperation() resolves a result type from the operands via a precedence table,
				performs the operation in 64-bit signed/unsigned (or double) space, and boxes the
				result back into the resolved type. Integer divide and modulus by zero, and integer
				add/subtract/multiply whose exact result does not fit the 64-bit result type, return
				NaN rather than trapping or wrapping. A nil operand or an operand with an objCType
				outside the precedence table raises NSInvalidArgumentException.
*/
...
int64_t pow_int64(int64_t base, uint64_t exponent)
{
	...
	// Work in the unsigned domain: |INT64_MIN| has no positive int64_t.
	...
	// Negate in the unsigned domain: result may equal |INT64_MIN|, which has no positive int64_t.
	return negative_result ? (int64_t)(0ULL - result) : (int64_t)result;
}
...
// Encodes a double into a packed integer using a configurable IEEE 754-style format
// (1 sign bit, exponentBits exponent, mantissaBits mantissa). Ported from Prado's
// TBitHelper::floatToFpXX. With the defaults (exponentBits 5, mantissaBits 10, auto bias)
// this is the standard fp16/half-float encoding.
int64_t floatToFpXX(double value, int exponentBits, int mantissaBits, int exponentBias, BOOL IEEEConformance)
{
	if (exponentBits == 0) { exponentBits = 5; }
	if (mantissaBits == 0) { mantissaBits = 10; }
	if (exponentBits <= 0 || mantissaBits <= 0 || (exponentBits + mantissaBits + 1) > 64) {
		return 0;
	}
	int64_t exponentMaxValue = (1LL << exponentBits) - 1;
	if (exponentBias == INT_MIN) {
		exponentBias = (int)(exponentMaxValue >> 1);
	}
	if (exponentBias < 0 || exponentBias > exponentMaxValue) {
		return 0;
	}

	int sign = signbit(value) ? 1 : 0;	// captures -0.0 and -INFINITY, unlike value < 0
	value = fabs(value);
	int64_t exponent = 0;
	int64_t mantissa = 0;

	if (IEEEConformance && isnan(value)) {
		exponent = exponentMaxValue;
		mantissa = 1LL << (mantissaBits - 1);	// quiet-NaN payload
	} else if (IEEEConformance && (isinf(value) ||
			   value >= pow(2.0, (double)((exponentMaxValue - 1) - exponentBias)) * (double)(1LL << mantissaBits))) {
		...
		if (IEEEConformance && exponent <= 0) {
			// Subnormal: encode with the implicit leading bit absent.
			mantissa = (int64_t)llround(value / pow(2.0, (double)(1 - exponentBias - mantissaBits)));
			exponent = 0;
		} else if (IEEEConformance && exponent >= exponentMaxValue) {
			exponent = exponentMaxValue;	// overflow to infinity
			mantissa = 0;
		} else {
			// Normal number. Without IEEE conformance there are no subnormals or infinities,
			// so exponent 0 and exponentMaxValue are normal exponents and the value saturates.
			if (exponent < 0) {
				exponent = 0;
			} else if (exponent > exponentMaxValue) {
				exponent = exponentMaxValue;
			}
			int64_t totalMantissaValues = (1LL << mantissaBits);
			mantissa = (int64_t)llround((value / pow(2.0, (double)(exponent - exponentBias)) - 1.0) * (double)totalMantissaValues);
			if (mantissa < 0) {
				mantissa = 0;
			} else if (mantissa >= totalMantissaValues) {
	...
	return ((((int64_t)sign << exponentBits) | exponent) << mantissaBits) | mantissa;
}
...
NSNumber* numberOperation(NSNumberMathOperation operation, NSNumber *first, NSNumber *second)
{
	if (first == nil || second == nil) {
		[NSException raise:NSInvalidArgumentException
					format:@"numberOperation: operands must be non-nil (first %@, second %@)", first, second];
	}

	static NSArray<NSString *> *typeOrder = nil;
	static dispatch_once_t typeOrderOnce;
	dispatch_once(&typeOrderOnce, ^{
...
#define kOperationWithDouble			12
	...
	if (firstIndex == NSNotFound || secondIndex == NSNotFound) {
		[NSException raise:NSInvalidArgumentException
					format:@"numberOperation: unsupported objCType (first '%@', second '%@')", firstType, secondType];
	}
	...
	if (highestIndex <= 10) {
		...
		// Perform the mathematical operation. The overflow builtins compute the exact result
		// and report whether it fits the result operand's type, including mixed-sign operands.
		...
		switch (operation) {
			case NSNumberMathOperationAdd:
				if (_firstUnsigned && _secondUnsigned) {
					overflow = __builtin_add_overflow(uFirstValue, uSecondValue, &ullResult);
				} else if (_secondUnsigned) {
					overflow = __builtin_add_overflow(firstValue, uSecondValue, &ullResult);
				} else if (_firstUnsigned) {
					overflow = __builtin_add_overflow(uFirstValue, secondValue, &ullResult);
				} else {
					overflow = __builtin_add_overflow(firstValue, secondValue, &llResult);
				}
				break;
			case NSNumberMathOperationSubtract:
				if (_firstUnsigned && _secondUnsigned) {
					overflow = __builtin_sub_overflow(uFirstValue, uSecondValue, &ullResult);
				} else if (_secondUnsigned) {
					overflow = __builtin_sub_overflow(firstValue, uSecondValue, &ullResult);
				} else if (_firstUnsigned) {
					overflow = __builtin_sub_overflow(uFirstValue, secondValue, &ullResult);
				} else {
					overflow = __builtin_sub_overflow(firstValue, secondValue, &llResult);
				}
				break;
			case NSNumberMathOperationMultiply:
				if (_firstUnsigned && _secondUnsigned) {
					overflow = __builtin_mul_overflow(uFirstValue, uSecondValue, &ullResult);
				} else if (_secondUnsigned) {
					overflow = __builtin_mul_overflow(firstValue, uSecondValue, &ullResult);
				} else if (_firstUnsigned) {
					overflow = __builtin_mul_overflow(uFirstValue, secondValue, &ullResult);
				} else {
					overflow = __builtin_mul_overflow(firstValue, secondValue, &llResult);
				}
				break;
			...
			case NSNumberMathOperationModulus:
				// Integer remainder by zero is undefined behavior (SIGFPE); mirror integer
				// divide-by-zero and return NaN rather than crashing.
				if ((_secondUnsigned && uSecondValue == 0) || (!_secondUnsigned && secondValue == 0)) {
					return @(NAN);
				}
				if (_firstUnsigned && _secondUnsigned) {
			...
			case NSNumberMathOperationPower:
				if (!_secondUnsigned && secondValue < 0) {
					double base = _firstUnsigned ? (double)uFirstValue : (double)firstValue;
					return [NSNumber numberWithDouble:pow(base, (double)secondValue)];
				}
				if (_firstUnsigned && _secondUnsigned) {
		...
		if (highestIndex == kOperationWithChar) {
		...
		switch (operation) {
			...
			case NSNumberMathOperationXor: {
				...
				// Casting a double >= 2^31 to int is undefined and differs across ABIs.
				// Saturate to int64 (a defined conversion) first, then XOR.
				long long a = (firstValue  >= 9223372036854775807.0) ? INT64_MAX
							: (firstValue  <= -9223372036854775808.0) ? INT64_MIN : (long long)firstValue;
				long long b = (secondValue >= 9223372036854775807.0) ? INT64_MAX
							: (secondValue <= -9223372036854775808.0) ? INT64_MIN : (long long)secondValue;
				result = (double)(a ^ b);
				break;
			}
		}

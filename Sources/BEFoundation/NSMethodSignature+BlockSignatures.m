/*!
 @file			NSMethodSignature+BlockSignatures.m
 @copyright		-© 2025 Delicense - @belisoful. All rights released.
 @date			2025-01-01
 @author		belisoful@icloud.com
 @abstract		Implements the NSMethodSignature (BlockSignatures) category and BEMethodSignatureHelper.
 @discussion	Extracts a block's @encode signature and transforms it into an NSMethodSignature
				suitable for dynamic method implementation (dropping the leading block pointer and,
				optionally, injecting a SEL _cmd argument).
*/
...
+ (NSString *)parseTypeAtPointer:(const char **)pointer
{
	...
	// Consume any leading type qualifiers (const, in, inout, out, bycopy, byref, oneway,
	// _Atomic) so the qualifier and the type it qualifies return as one token.  Falling
	// through to the default case instead emits a bare qualifier, and the offset scan then
	// swallows the base type, dropping the argument from the signature entirely.

 ...
 *   the default base filename for an entry's paired .cache and .meta files
 *   (a set fileNameBlock may substitute its own; see baseFileNameForKey:hashName:).
 *
 * @discussion
 *   Hashing the key's archived bytes provides three properties a persistent
 *   cache requires:
 *
...
/**
 * @abstract  Returns whether @p name is usable as an entry's base file name.
 *
 * @discussion
 *   A base name must stay inside cacheDirectory when appended as a path
 ...
 *              so the byte count measures what the file system sees).
 *
 * @return @c YES if the name is a safe, writable single path component.
 */
static BOOL BEIsSafeBaseFileName(NSString * _Nullable name) {
	...
	NSUInteger extension = BE_FILE_CACHE_EXTENSION.length + 1;   // ".cache"
	if (bytes == 0 || bytes + extension > NAME_MAX) return NO;
	return YES;
}
 ...
 * @param b  Second path.
 *
 * @return @c YES if the paths are identical strings, or if both exist and
 *         resolve to one file.
 */
static BOOL BEIsSameFile(NSString *a, NSString *b) {
 ...
 *
 *   Two pairs can decode to one key when a fileNameBlock rename was interrupted
 *   between writing the new pair and deleting the old.  The scan keeps the pair
 *   with the newer @c dateStored, deletes the loser's files (guarded by
 *   @c BEIsSameFile against path aliases), and counts the key once.
 *
...
- (void)scanMetaFilesForIndex {
	...
	for (NSString *f in files) {
		...
		// Both files share the same base name; derive the sibling .cache path
		...
		// Two pairs can decode to one key when a rename was interrupted
		// between writing the new pair and deleting the old.  Keep the newer
		// payload, delete the loser's files, and count the key once.
		NSDictionary *existing = _diskMeta[(id)item.key];
		if (existing) {
			BOOL adoptedNewer =
				[item.dateStored compare:existing[kMetaDate]] == NSOrderedDescending;
			NSString *keepObj  = adoptedNewer ? objectFilePath : existing[kMetaObjectFile];
			NSString *keepMeta = adoptedNewer ? metaFilePath   : existing[kMetaMetaFile];
			NSString *loseObj  = adoptedNewer ? existing[kMetaObjectFile] : objectFilePath;
			NSString *loseMeta = adoptedNewer ? existing[kMetaMetaFile]   : metaFilePath;
			if (!BEIsSameFile(loseObj,  keepObj))  [fm removeItemAtPath:loseObj  error:nil];
			if (!BEIsSameFile(loseMeta, keepMeta)) [fm removeItemAtPath:loseMeta error:nil];
			if (!adoptedNewer) continue;
			_diskTotalCost -= [existing[kMetaCost] unsignedIntegerValue];
 ...
 *   - A pair whose key the index already tracks under another name (a
 *     fileNameBlock rename interrupted between writing the new pair and
...
- (void)reconcileWithDirectoryOnQueue {
	...
	// Base names of the .cache and .meta files present on disk.  All base
	// names in this pass use decomposed form: directory listings return it,
	// and recorded paths from an earlier release may be composed, so both
	// sides normalize before set membership is tested.
	...
	for (NSString *base in cacheBases) {
		...
		// The pair can belong to a key the index already tracks under another
		// name (a rename interrupted between writing the new pair and deleting
		// the old).  Keep the newer payload, delete the loser's files, and
		// count the key once.
		NSDictionary *existing = _diskMeta[(id)item.key];
		if (existing) {
			BOOL adoptedNewer =
...
// ---------------------------------------------------------------------------
#pragma mark - Private – file naming
// ---------------------------------------------------------------------------

/**
 * @method baseFileNameForKey:hashName:
 *
 * @abstract
 *   Returns the base file name for @p key's on-disk pair, applying
 *   @c fileNameBlock when one is set.
 *
 * @discussion
 ...
 * @param hashName  The key's SHA-256 digest name from @c BEHashForKey.
 *
 * @return The base name for the entry's files.
 */
- (NSString *)baseFileNameForKey:(id<NSCopying, NSSecureCoding>)key
						hashName:(NSString *)hashName {
	BEFileCacheFileNameBlock block = self.fileNameBlock;   // atomic copy-in
	if (!block) return hashName;

	NSString *custom = block(key, hashName);
	if ([custom isKindOfClass:NSString.class]) {
		custom = custom.decomposedStringWithCanonicalMapping;
	}
	if (BEIsSafeBaseFileName(custom)) return custom;

	NSLog(@"[BEFileCache] fileNameBlock returned an unusable name (%@); "
		  @"using the hash name", custom);
	return hashName;
}
 ...
 *
 * @discussion
 *   Two files are written per entry, sharing the base name from
 ...
 *
 *   The file pair is resolved on @c diskQueue inside the same critical section
 *   that updates @c diskMeta, so the collision check reads a stable index.  A
 *   fileNameBlock name that matches another key's recorded base name
 *   (case-insensitively, in decomposed form) falls back to the digest name;
 *   honoring it would point two index entries at one file pair.  On overwrite,
 *   a prior pair recorded at different paths is deleted; @c BEIsSameFile
 *   guards against deleting a path alias of the files just written.
 *
 *   The object is also stored into the memory tier inside the same critical
 *   section, so the two tiers cannot diverge; on a failed disk write the prior
 *   disk entry is purged so an older value cannot resurface after memory
 *   eviction.
 *
...
- (BOOL)writeToDisk:(id<NSSecureCoding>)obj
	...
	// Derive the base name for the payload/sidecar pair.  The digest is kept
	// alongside the block's substitute for the collision fallback below.
	NSString *hashName = BEHashForKey(key);
	if (!hashName) return NO;    // key archiving failed
	...
	dispatch_sync(_diskQueue, ^{
		...
		// Resolve the file pair inside the critical section so the collision
		// check reads a stable diskMeta.  A custom name another key's entry
		// already records falls back to the digest: honoring it would point
		// two index entries at one file pair and corrupt both.  The comparison
		// is case-insensitive to catch aliasing on case-insensitive volumes;
		// recorded names are already in decomposed form.
		NSString *useBase = base;
		if (![useBase isEqualToString:hashName]) {
			for (id existingKey in self->_diskMeta) {
				if ([existingKey isEqual:key]) continue;
				NSString *otherBase =
					[self->_diskMeta[existingKey][kMetaObjectFile]
						lastPathComponent].stringByDeletingPathExtension;
				if ([otherBase caseInsensitiveCompare:useBase] == NSOrderedSame) {
					NSLog(@"[BEFileCache] fileNameBlock name '%@' collides with "
						  @"another entry; using the hash name", useBase);
					useBase = hashName;
					break;
				}
			}
		...
		NSString *objectFilePath =
			[self->_cacheDirectory stringByAppendingPathComponent:
			 [useBase stringByAppendingPathExtension:BE_FILE_CACHE_EXTENSION]];
		NSString *metaFilePath =
			[self->_cacheDirectory stringByAppendingPathComponent:
			 [useBase stringByAppendingPathExtension:BE_FILE_CACHE_META_EXTENSION]];
		if (!objectFilePath || !metaFilePath) {
			// Unreachable for names BEIsSafeBaseFileName accepted; kept so a
			// nil path can never reach writeToFile:, which raises on nil.
			[self removeDiskEntryOnQueue:key notifyDelegate:NO];
			return;
		}
		...
		if (prev) {
			// A fileNameBlock can rename a key between writes; delete a pair
			// left at the old name or the launch reconciliation pass would
			// re-adopt it as a second, stale entry for the same key.  The
			// same-file guard keeps a path alias of the pair just written
			// (normalization or case) from deleting the live files.
			NSString *prevObjectFile = prev[kMetaObjectFile];
			NSString *prevMetaFile   = prev[kMetaMetaFile];
			if (!BEIsSameFile(prevObjectFile, objectFilePath)) {
				[[NSFileManager defaultManager]
					removeItemAtPath:prevObjectFile error:nil];
			}
			if (!BEIsSameFile(prevMetaFile, metaFilePath)) {
				[[NSFileManager defaultManager]
					removeItemAtPath:prevMetaFile error:nil];
			}
			self->_diskTotalCost -= [prev[kMetaCost] unsignedIntegerValue];
		} else {
 ...
 *   score; ties break by least-recently-used first, then larger @c cost, so a
 *   full cache still trims.  Recency-first ties keep trimming
 *   least-recently-used at @c evictionBalance 1 with default retention costs,
 *   where every score ties at 1.0.
 *
 * @note Must be called on @c diskQueue.
 *
...
- (NSArray<NSDictionary *> *)scoredEvictionSnapshot {
	...
	[scored sortUsingComparator:^NSComparisonResult(NSDictionary *a,
		...
		// Tiebreak: least-recently-used first, then larger cost.  Recency
		// first keeps the retentionCost-defaults-to-cost contract at
		// evictionBalance 1, where every default-retention score ties at 1.0.
		NSComparisonResult access = [a[kMetaAccess] compare:b[kMetaAccess]];
		if (access != NSOrderedSame) return access;

...
- (BOOL)hasMutability {
	// __NSCFString backs BOTH mutable and immutable NSStrings and inherits NSMutableString's
	// declared conformance, so neither the backing class nor protocol conformance can tell
	// them apart. Constant strings are the one case that is decidable, so they answer NO and
	// every other __NSCFString answers YES; an immutable NSString built at runtime is
	// therefore reported mutable. That ambiguity is inherent to the class cluster.

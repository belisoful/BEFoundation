...
- (BOOL)isIndexedSubscriptNumeric {
	...
	if (value != nil) {
		...
		// Memoize only a decision the keys actually support.  Caching the default for an
		// empty receiver would freeze it: a later read reports the empty-dictionary answer
		// instead of the one its keys imply, and indexed writes then add a numeric key
		// beside an existing string key for the same index.
		if (self.count > 0) {

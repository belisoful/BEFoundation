...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location
{
	...
	// Read the payload before the file callbacks run.  The documented pattern for
	// tempCompletionBlock moves the file out of the temporary location, and URLSession
	// deletes whatever remains once this method returns, so a later read finds nothing.
	BOOL delegateWantsData = [delegate respondsToSelector:@selector(downloadDataComplete:urlResponse:)];
	BOOL deliversData = self.allowBothCompletions && (self.dataCompletionBlock || delegateWantsData);
	if (deliversData) {

/*!
 @file			NSObject+DynamicMethodsHelpers.m
 @copyright		-© 2025 Delicense - @belisoful. All rights released.
 @date			2025-01-01
 @author		belisoful@icloud.com
 @abstract		Helper routines backing the NSObject (DynamicMethods) runtime injection system.
 @discussion	Provides protocol enumeration, swizzle installation, and the block→invocation
				bridging (invocableMethodSignatureFromBlock: and mutateInvocation:withMeta:) used to
				dispatch a dynamic method's block, optionally injecting a SEL _cmd argument.
*/
...
- (int)swizzleMethodsOnClass:(Class)targetClass
{
	...
	if (didAddMethod) {
		...
		// class_getInstanceMethod searches the superclass chain, and the swizzle selectors
		// live in the NSObject category, so swizzleMethod is usually NSObject's Method.
		// Exchanging it writes this class's IMP into a slot every other class inherits.
		// Give targetClass its own copy first, then exchange that.  (originalMethod is
		// guaranteed local here: class_addMethod failing is exactly that condition.)
		class_addMethod(targetClass,
						swizzleSelector,
						method_getImplementation(swizzleMethod),
						swizzledTypeEncoding);
		swizzleMethod = class_getInstanceMethod(targetClass, swizzleSelector);
		if (!swizzleMethod) {
			return 0;
		}
...
+ (nullable NSMethodSignature *)invocableMethodSignatureFromBlock:(nonnull id)block
{
	if(!block) {
		[NSException raise:NSInvalidArgumentException
					format:@"*** -[%@ %@]: nil argument", NSStringFromClass(NSMethodSignature.class), NSStringFromSelector(_cmd)];
	} else if (![block isKindOfClass:NSClassFromString(@"NSBlock")]) {
		[NSException raise:NSInvalidArgumentException
					format:@"*** -[%@ %@]: Argument is not a NSBlock", NSStringFromClass(NSMethodSignature.class), NSStringFromSelector(_cmd)];
	}

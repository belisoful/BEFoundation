...
- (NSString *)setSimpleRegistryUUID:(NSString *)uuid forObject:(id<BERegistryProtocol, NSObject>)object
{
	...
	@synchronized (registryTable) {
		// Report the prior UUID for a clear as well as a change: the caller removes the
		// table entry keyed by it, and leaving that entry behind strands the object under
		// a UUID it no longer carries.

...
+ (SRational32)rationalize:(Float64)number tolerance:(double)tolerance
{
	...
	do {
		...
		if (h > ((double)(0x7FFFFFFF) + offset) || k > (double)0x7FFFFFFF) {
			...
			k = lk;
			if (k == 0.0) {
				// The first iteration overflowed, so lh/lk are still the 1/0 seed and
				// reverting to them would return a zero divisor. Saturate instead, matching
				// how the infinite inputs are handled.
				h = (double)0x7FFFFFFF + offset;
				k = 1.0;
			}
			break;
		}

/*!
 ...
 @discussion	This header provides a category extension for NSCoder that encodes and decodes
				half-precision floating-point values (_Float16). NSCoder has no built-in
				half-precision methods; this extension stores and reads the value's raw bytes.
 */
...
/*!
 ...
 @param			key The string key to associate with the encoded value. The same key retrieves
				the value during decoding. Must not be nil.
...
/*!
 ...
 @discussion	This method retrieves and decodes a _Float16 value that was previously stored
				...
				_Float16, the method returns 0. This matches the contract of NSCoder's other
				scalar decoders (decodeFloatForKey:, decodeIntForKey:), which default to
				zero for a missing key. As with those, a stored 0 is indistinguishable
				from a missing key; use -containsValueForKey: when you need to tell them apart.
				A value encoded as NaN round-trips and decodes to NaN.

...
@discussion The BEMetalHelper class converts Metal textures to BEImage objects and grayscale data to RGB using Apple's vImage framework.
*/
...
@discussion BEMetalHelper provides static methods for converting Metal textures to BEImage objects and grayscale data to RGB using Apple's vImage framework. The class supports various pixel formats including 8-bit, 16-bit float, and 32-bit float textures.
...
@return A BEImage object containing the converted texture data, or nil if the conversion fails, the texture format is unsupported, an allocation fails, or the dimensions would overflow.
...
@note The produced image is interpreted in the kCGColorSpaceGenericRGBLinear color space. If your texture holds sRGB-/gamma-encoded *Unorm content, the result looks incorrect; convert or reinterpret accordingly.
...
@discussion This method converts single-channel 8-bit grayscale data to 4-channel XRGB format by duplicating the grayscale values across the red, green, and blue channels using Apple's vImage framework.
...
@discussion This method converts single-channel 16-bit half-precision float grayscale data to 4-channel 32-bit float RGB format. The conversion process involves two steps: first converting 16-bit half-precision floats to 32-bit floats, then duplicating the grayscale values across RGB channels using Apple's vImage framework.
...
@discussion This method converts single-channel 32-bit float grayscale data to 4-channel 32-bit float RGB format by duplicating the grayscale values across the red, green, and blue channels using Apple's vImage framework.
